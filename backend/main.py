"""
backend/main.py
FastAPI server for ShockSmart predictions.
"""
...
def engineer(raw: dict) -> np.ndarray:
    """Apply the same feature engineering as training. Returns a 2D numpy array."""
    # Fill in raw values from request, fallback to median
    d = {k: float(raw.get(k, MEDIANS.get(k, 0))) for k in RAW_FEATURES}
    ...
    # Z-scored features (use median as proxy for mean for single-row prediction)
    for col in ["systolic_bp", "resting_hr", "age", "weight_kg", "diastolic_bp"]:
        mu = MEDIANS.get(col, d[col])
        d[f"{col}_z"] = (d[col] - mu) / 20.0  # rough std estimate

    # Build row in the exact feature order the model was trained with
    row = np.array([[float(d.get(f, 0)) for f in FEATURES]])
    return row
